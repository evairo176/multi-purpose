...
<div>
        ...
        <div class="card-body table-responsive p-0">
            <table class="table table-hover ">
                ...
                <tbody wire:loading.class="text-muted">
                    ...
                    <tr>
                        <td>{{($desas->currentPage() - 1) * $desas->perPage() + $loop->iteration}}</td>
                        <td>{{$desa->id}}</td>
                        <td>{{$desa->name_desa}}</td>
                        <td class="text-center">
                            <div class="btn-group">
                                <button type="button" class="btn btn-primary"><i class="fas fa-eye"></i></button>
                                <button wire:click.prevent="edit({{$desa->id}})" type="button" class="btn btn-warning"><i class="fas fa-edit"></i></button>
                                <button wire:click.prevent="confirmationDeleteUser({{$desa->id}})" type="button" class="btn btn-danger"><i class="fas fa-trash"></i></button>
                            </div>
                        </td>
                    </tr>
                    @empty
    ...
    <div wire:ignore.self class="modal fade" id="form" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <form wire:submit.prevent="{{$showEditModal ? 'updateDesa' : 'createDesa'}}">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('id_kecamatan') is-invalid @enderror" id="id_kecamatan" wire:model.defer="state.id_kecamatan">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" wire:model.defer="state.name_desa" class="form-control @error('name_desa') is-invalid @enderror" id="name_Desa" aria-describedby="DesaHelp" placeholder="Masukan nama Desa">
                            @error('name_desa')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
